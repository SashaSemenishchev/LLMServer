...
extern "C" {
    ...
    typedef struct falcon_evaluation_config {
        // mandatory configuration
        int n_tokens;       // number of tokens to process
        int n_past;         // number of tokens in kv cache past
        int n_threads;      // number of threads available

        // optional
        const char *cgraph_fname; // path to the cgraph export file
        int n_max_real_ctx; // the actual max achievable context given all parameters (-c, -n, -enc, -sys)

        // debug related
        int debug_timings;  // 0 (none), 1(first token), 2(first,last), 3(every token)
    } falcon_evaluation_config;

    typedef struct falcon_context_params {
    ...
    }falcon_context_params;

    // model file types
    typedef enum llama_ftype {
    ...
    LLAMA_API void falcon_context_set_buffers(struct falcon_context *ctx, int n_batch, int n_ctx);
    LLAMA_API struct falcon_model * falcon_get_falcon_model(struct falcon_context * ctx);
